...
function dl_not_found(string $message = 'Não Encontrado'): void
{
...
function dl_handle_black_step_route(string $reqPath, array $mimeTypes): bool
{
    ...
    if (empty($click)) {
        dl_not_found('Clique não encontrado');
    }
    ...
    if ($stepIndex > $maxReachedStep) {
        dl_not_found('Etapa ainda não alcançada');
    }

    $campId = (int)$click['campaign_id'];
    $settings = $db->get_campaign_settings($campId);
    if (empty($settings)) {
        dl_not_found('Campanha não encontrada');
    }
    ...
    if ($flow === null || !isset($flow->steps[$stepIndex])) {
        dl_not_found('Fluxo ou etapa não encontrado');
    }
    ...
    if (!$step->isFolder()) {
        dl_not_found('Etapa não é uma pasta');
    }

    $landingBase = __DIR__ . '/' . get_cache_path('landingFolder') . '/' . $variant;
    $realBase = realpath($landingBase);
    if ($realBase === false) {
        dl_not_found('Pasta não encontrada');
    }
