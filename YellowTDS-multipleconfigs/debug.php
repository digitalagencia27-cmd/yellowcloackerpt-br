...
class DebugMethods
{
    ...
    public static function check_php():void
    {
        ...
        if (version_compare($ver, '8.2.0', '<'))
            die("A versão do PHP deve ser 8.2.0 ou superior! Altere sua versão do PHP e retorne.");
    }
    
    public static function check_sqlite():void
    {
        if (!extension_loaded('sqlite3')) 
            die("Extensão SQLite NÃO ENCONTRADA! Use outro servidor ou habilite o SQLite.");
    }
    ...
    public static function check_curl():void
    {
        ...
        if (!extension_loaded('curl')) 
            die("Extensão cURL NÃO ENCONTRADA! Use outro servidor ou habilite o cURL.");
    }
    
    public static function check_dirs():void
    {
        if (!is_writable(__DIR__)) 
            die("PHP não tem permissão de escrita para o diretório atual. Altere as permissões de acesso!");
    }
}
if (DebugMethods::on()){
