...
if ($clickid === '') {
    ...
    $msg = 'Nenhum clickid encontrado! URL: ' . $curLink;
...
if ($status === '') {
    http_response_code(500);
    $msg = 'Nenhum status encontrado! URL: ' . $curLink;
    add_log('postback', $msg);
    echo $msg;
    exit;
}
...
if ($payout === '') {
    http_response_code(500);
    $msg = 'Nenhum payout encontrado! URL: ' . $curLink;
    add_log('postback', $msg);
    echo $msg;
    exit;
}
...
if (empty($click)) {
    http_response_code(500);
    $msg = 'Nenhum dado de clique para clickid ' . $clickid . ' encontrado! URL: ' . $curLink;
    add_log('postback', $msg);
    echo $msg;
    exit;
}
...
if ($inner_status === '') {
    http_response_code(500);
    $msg = 'Status ' . $status . ' é desconhecido! URL: ' . $curLink;
    add_log('postback', $msg);
    echo $msg;
    exit;
}
...
if ($updated) {
    ...
    $msg = 'Postback para clickid ' . $clickid . ' com status ' . $status . ' e payout ' . $payout . ' ' . $currency . ' aceito.';
    add_log('postback', $msg);
    echo $msg;
} else {
    http_response_code(404);
    $msg = 'Postback para clickid ' . $clickid . ' com status ' . $status . ' e payout ' . $payout . ' ' . $currency . ' NÃO aceito! CLICKID NÃO ENCONTRADO.';
    add_log('postback', $msg);
    echo $msg;
}
