...
class MacrosProcessor
{
    ...
    private function getCustomClickParam($macro): string|bool
    {
        if (!empty($this->clickParams)) {
            ...
            // Also check in 'qs' (query string) for UTM parameters and other URL params
            if (isset($this->clickParams['qs']) && is_array($this->clickParams['qs']) && array_key_exists($cmacro, $this->clickParams['qs'])) {
                return (string)$this->clickParams['qs'][$cmacro];
            }
        }
