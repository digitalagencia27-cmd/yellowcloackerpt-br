...
class AutoUpdater {
    ...
    public function checkForUpdates(): bool {
        try {
            ...
            if ($response === false) {
                throw new Exception("Falha ao buscar informações de versão");
            }

            $fileInfo = json_decode($response, true);
            if (!$fileInfo || !isset($fileInfo['content'])) {
                throw new Exception("Informação de versão inválida");
            }
    ...
    private function convertVersionToTimestamp(string $version): int {
        ...
        if (count($parts) !== 3) {
            throw new Exception("Formato de versão inválido");
        }
    ...
    public function update(): array {
        ...
        try {
            ...
            if (!copy(self::SETTINGS_FILE, $settingsBackup)) {
                throw new Exception("Falha ao fazer backup do settings.php");
            }
            ...
            if (!$extractedDir) {
                throw new Exception("Falha ao localizar arquivos extraídos");
            }
            ...
            $result['success'] = true;
            $result['message'] = "Atualizado com sucesso para a versão " . $this->latestVersion;
        } catch (Exception $e) {
            $result['message'] = "Falha na atualização: " . $e->getMessage();
