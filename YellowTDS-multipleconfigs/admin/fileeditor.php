...
function get_lcache_dir(): string
{
    ...
    if ($dir === false) {
        fe_error('A pasta de destino não existe');
    }
    return $dir;
}
...
function safe_path(string $lcacheDir, string $folder, string $relativePath = ''): string
{
    ...
    if ($real !== false) {
        ...
        if ($folderReal === false || !str_starts_with($real, $folderReal)) {
            fe_error('Acesso negado: caminho fora do diretório permitido');
        }
        return $real;
    }
    ...
    if ($parent === false) {
        fe_error('Diretório pai não existe');
    }
    $folderReal = realpath($lcacheDir . DIRECTORY_SEPARATOR . $folder);
...
if (!preg_match('/^[a-zA-Z0-9_\-\.]+$/', $folder)) {
    fe_error('Nome de pasta inválido');
}
...
if (!is_dir($folderPath)) {
    fe_error('A pasta "' . $folder . '" não existe');
}
...
switch ($action) {
    ...
    case 'read':
        $file = $_REQUEST['file'] ?? '';
        if (empty($file)) fe_error('Nenhum arquivo especificado');
        $path = safe_path($lcacheDir, $folder, $file);
        if (!is_file($path)) fe_error('Arquivo não encontrado');
        $content = file_get_contents($path);
        if ($content === false) fe_error('Não foi possível ler o arquivo');
    ...
    case 'save':
        ...
        if (file_put_contents($path, $content) === false) {
            fe_error('Não foi possível escrever no arquivo');
        }
    ...
    case 'create':
        ...
        if (empty($file)) fe_error('Nenhum nome especificado');
        $path = safe_path($lcacheDir, $folder, $file);
        if (file_exists($path)) fe_error('Já existe');
        if ($type === 'dir') {
            if (!mkdir($path, 0755, true)) fe_error('Não foi possível criar o diretório');
        } else {
            $dir = dirname($path);
            if (!is_dir($dir)) @mkdir($dir, 0755, true);
            if (file_put_contents($path, '') === false) fe_error('Não foi possível criar o arquivo');
        }
        echo json_encode(['error' => false, 'result' => 'OK']);
        break;
    ...
    case 'delete':
        ...
        if (empty($file)) fe_error('No file specified');
        $path = safe_path($lcacheDir, $folder, $file);
        if (!file_exists($path)) fe_error('Não encontrado');
        if (is_dir($path)) {
            if (!delete_dir_recursive($path)) fe_error('Não foi possível excluir o diretório');
        } else {
            if (!unlink($path)) fe_error('Não foi possível excluir o arquivo');
        }
        echo json_encode(['error' => false, 'result' => 'OK']);
        break;
    ...
    case 'rename':
        ...
        if (empty($file) || empty($newName)) fe_error('Parâmetros faltando');
        ...
        if (file_exists($newPath)) fe_error('O destino já existe');
        if (!rename($oldPath, $newPath)) fe_error('Não foi possível renomear');
    ...
    case 'upload':
        ...
        if (empty($_FILES['file']) || $_FILES['file']['error'] !== UPLOAD_ERR_OK) {
            fe_error('Nenhum arquivo enviado');
        }
        ...
        if (!move_uploaded_file($_FILES['file']['tmp_name'], $targetPath)) {
            fe_error('Não foi possível salvar o arquivo enviado');
        }
        echo json_encode(['error' => false, 'result' => 'OK', 'file' => $targetRel]);
        break;

    default:
        fe_error('Ação desconhecida');
}
