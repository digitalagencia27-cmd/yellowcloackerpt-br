...
if (!$passOk)
    return send_camp_result("Erro: verificação de senha não passou!",true);
...
switch ($action) {
    case 'add':
        ...
        if ($campId===false)
            return send_camp_result("Erro ao adicionar nova campanha!",true);
        break;
    case 'dup':
        if (empty($name)) {
            return send_camp_result("Erro: nome da campanha não pode estar vazio!", true);
        }
        $clonedId = $db->clone_campaign($campId);
        if ($clonedId===false)
            return send_camp_result("Erro ao duplicar campanha!",true);
        if (!empty($name)) {
            $renRes = $db->rename_campaign((int)$clonedId, $name);
            if ($renRes === false) {
                return send_camp_result("Erro ao renomear campanha clonada!", true);
            }
        }
        break;
    case 'del':
        ...
        if ($delRes===false)
            return send_camp_result("Erro ao excluir campanha!",true);
        break;
    case 'ren':
        $renRes = $db->rename_campaign($campId, $name);
        if ($renRes===false)
            return send_camp_result("Erro ao renomear campanha!",true);
        break;
    case 'save':
        ...
        if($saveRes===false)
            return send_camp_result("Erro ao salvar campanha!",true);
        break;
    default:
        return send_camp_result("Erro: ação inválida!",true);
}
