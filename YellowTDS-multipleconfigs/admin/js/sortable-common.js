...
function addFilterRowToDOM(field, operator, value) {
    ...
    row.innerHTML = `
        <div class="filter-row-main">
            ...
            <input type="text" class="form-control filter-value" style="display:${isParam || !needsValue ? 'none' : ''}" placeholder="valor" value="${isParam ? '' : displayValue}">
            <button class="btn btn-sm btn-outline-danger filter-remove" title="Remover">&times;</button>
        </div>
        <div class="filter-row-param" style="display:${isParam ? '' : 'none'}">
            <span class="filter-param-label">param.</span>
            <input type="text" class="form-control filter-param-key" placeholder="chave" value="${paramKey}">
            <select class="form-control filter-op-param">${opOptions}</select>
            <input type="text" class="form-control filter-value-param" ${needsValue ? '' : 'style="display:none;"'} placeholder="valor" value="${isParam ? displayValue : ''}">
        </div>
    `;
