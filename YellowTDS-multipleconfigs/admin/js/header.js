        ...
        updateBasesLink.addEventListener('click', async function(e) {
            ...
            try {
                ...
                if (!jsr.error) {
                    ...
                    alert('Erro ao atualizar geobases:\n' + jsr.result);
                }
            } catch (error) {
                alert('Erro ao atualizar geobases:\n' + error);
            } finally {
...
async function checkForUpdates() {
    ...
    try {
        ...
        if (!result.success) {
            alert('Erro ao verificar atualizações: ' + result.message);
            return;
        }
        
        if (!result.hasUpdate) {
            alert('Seu sistema está atualizado!');
            return;
        }
        
        if (confirm(`Uma atualização para a versão ${result.version} está disponível. Deseja atualizar agora?`)) {
            const updateResult = await sendAutoupdateRequest('update');
            
            if (updateResult.success) {
                alert('Atualização bem-sucedida! A página será recarregada.');
                location.reload();
            } else {
                alert('Erro ao atualizar o sistema: ' + updateResult.error);
            }
        }
    } catch (error) {
        alert('Erro ao atualizar o sistema: ' + error);
    } finally {
        if (typingCleanup) typingCleanup();
        updateOverlay.style.display = 'none';
    }
