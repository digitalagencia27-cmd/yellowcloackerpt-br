...
export function handleStepAddExisting(e) {
    ...
    fetch('listfolders.php').then(function (r) { return r.json(); }).then(function (data) {
        ...
        if (!data.folders.length) { alert('Nenhuma pasta encontrada. Envie um ZIP primeiro.'); return; }
    ...
    }).catch(function (err) { btn.disabled = false; alert('Erro: ' + err); });
}
...
export function handleEditFolder(e) {
    ...
    if (!folderInput || !folderInput.value.trim()) {
        alert('Por favor, insira um nome de pasta primeiro.');
        return;
    }
    if (typeof window.openFileEditor === 'function') {
        window.openFileEditor(folderInput.value.trim());
    } else {
        alert('Editor de arquivos não carregado.');
    }
}
...
export function handleAddStep(e) {
    ...
    var navHtml = '<li class="step-nav-item" data-flow-index="' + fi + '" data-step-index="' + si + '"><a href="#sec-step-' + fi + '-' + si + '">&nbsp;&nbsp;&nbsp;&nbsp;Etapa ' + (si + 1) + '</a></li>';
...
export function handleAddFlow() {
    var flowName = prompt('Digite o nome do fluxo (não pode ser alterado depois):');
    ...
    for (var i = 0; i < existing.length; i++) {
        if (existing[i].value === flowName) {
            alert('O nome de fluxo "' + flowName + '" já existe. Escolha um nome diferente.');
            return;
        }
    }
    ...
    var rowHtml = '<div class="flow-list-row" data-flow-index="' + fi + '">' +
        ...
        '<a href="javascript:void(0)" class="btn btn-primary btn-sm flow-move-up" title="Mover para Cima">&uarr;</a> ' +
        '<a href="javascript:void(0)" class="btn btn-primary btn-sm flow-move-down" title="Mover para Baixo">&darr;</a> ' +
        '<a href="javascript:void(0)" class="btn btn-danger btn-sm flow-delete" title="Excluir"><i class="bi bi-trash"></i></a>' +
        '</div>';
