...
export function buildFolderRow(folderName, showWeight) {
    ...
    if (labelEl) labelEl.textContent = 'Pasta:';
...
export function buildStepListRow(fi, si) {
    ...
    div.innerHTML =
        '<span class="step-list-label">Etapa ' + (parseInt(si, 10) + 1) + '</span>' +
        '<span class="step-list-info">vazio</span>' +
        '<a href="javascript:void(0)" class="btn btn-primary btn-xs flow-move-step-up" title="Mover para Cima">&uarr;</a> ' +
        '<a href="javascript:void(0)" class="btn btn-primary btn-xs flow-move-step-down" title="Mover para Baixo">&darr;</a> ' +
        '<a href="javascript:void(0)" class="btn btn-danger btn-xs flow-remove-step" title="Excluir"><i class="bi bi-trash"></i></a>';
    return div;
}
...
export function renumberSteps(fi) {
    ...
    sections.forEach(function (sec, idx) {
        ...
        if (title) title.innerHTML = flowName + ' &rsaquo; Etapa ' + (idx + 1);
    ...
    sections.forEach(function (sec, idx) {
        ...
        if (!hint) {
            ...
            if (actionGroup && !isLast) {
                ...
                p.textContent = 'Apenas a última etapa pode usar redirecionamentos.';
...
export function updateStepListInfo(fi, si) {
    ...
    if (action === 'redirect') {
        ...
        infoEl.textContent = hosts.length ? hosts.join(', ') : 'redirecionar';
    } else {
        var folders = [];
        stepSec.querySelectorAll('.flow-step-folder').forEach(function (inp) {
            if (inp.value.trim()) folders.push(inp.value.trim());
        });
        infoEl.textContent = folders.length ? folders.join(', ') : 'vazio';
    }
}
