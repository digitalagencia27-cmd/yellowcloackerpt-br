async function campEditor(action, campId=null, name=null) {
    ...
    if (js.error)
        alert(`Ocorreu um erro: ${js.result}`);
    else
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    campMenuDropdown.innerHTML = `
        <div class="camp-menu-item btn-rename"><i class="bi bi-pencil-fill"></i> Renomear</div>
        <div class="camp-menu-item btn-clone"><i class="bi bi-copy"></i> Clonar</div>
        <div class="camp-menu-item btn-stats"><i class="bi bi-bar-chart-fill"></i> Estatísticas</div>
        <div class="camp-menu-item btn-allowed"><i class="bi bi-person-circle"></i> Permitidos</div>
        <div class="camp-menu-item btn-blocked"><i class="bi bi-ban"></i> Bloqueados</div>
        <div class="camp-menu-item btn-leads"><i class="bi bi-coin"></i> Leads</div>
        <div class="camp-menu-divider"></div>
        <div class="camp-menu-item btn-delete camp-menu-danger"><i class="bi bi-trash-fill"></i> Excluir</div>
    `;
    ...
    campMenuDropdown.addEventListener('click', async function(e) {
        ...
        if (menuItem.classList.contains('btn-rename')) {
            ...
            const newName = prompt("Digite o novo nome da campanha:", currentName);
            if (newName == null) return;
            const trimmedName = newName.trim();
            if (trimmedName) {
                await campEditor('ren', campaignId, trimmedName);
            } else {
                alert('O nome da campanha não pode estar vazio!');
            }
            return;
        }

        if (menuItem.classList.contains('btn-delete')) {
            if (confirm(`Tem certeza? A campanha ${campaignName} será excluída.`)) {
        ...
        if (menuItem.classList.contains('btn-clone')) {
            ...
            const newName = prompt("Digite o nome da campanha clonada:", defaultCloneName);
            if (newName == null) return;
            const trimmedName = newName.trim();
            if (!trimmedName) {
                alert('O nome da campanha não pode estar vazio!');
