...
function showEditorLoading(message) {
    ...
    var safe = escHtml(message || 'Carregando...');
    container.innerHTML =
...
function buildModal() {
    ...
    modal.innerHTML =
        '<div class="fe-modal-content">' +
            '<div class="fe-header">' +
                '<span class="fe-title">Editor de Arquivos: <span id="fe-folder-name"></span></span>' +
                '<div class="fe-header-actions">' +
                    '<span id="fe-current-file" class="fe-current-file"></span>' +
                    '<button class="btn btn-sm btn-success fe-btn" id="fe-save-btn" title="Salvar (Ctrl+S)"><i class="bi bi-floppy"></i> Salvar</button>' +
                    '<button class="btn btn-sm btn-secondary fe-btn" id="fe-close-btn"><i class="bi bi-x-lg"></i> Fechar</button>' +
                '</div>' +
            '</div>' +
            '<div class="fe-body">' +
                '<div class="fe-sidebar">' +
                    '<div class="fe-toolbar">' +
                        '<button class="btn btn-xs btn-outline-light fe-tb" id="fe-new-file" title="Novo Arquivo"><i class="bi bi-file-earmark-plus"></i></button>' +
                        '<button class="btn btn-xs btn-outline-light fe-tb" id="fe-new-folder" title="Nova Pasta"><i class="bi bi-folder-plus"></i></button>' +
                        '<button class="btn btn-xs btn-outline-light fe-tb" id="fe-upload-file" title="Enviar Arquivo"><i class="bi bi-upload"></i></button>' +
                        '<button class="btn btn-xs btn-outline-light fe-tb" id="fe-rename-btn" title="Renomear"><i class="bi bi-pencil"></i></button>' +
                        '<button class="btn btn-xs btn-outline-light fe-tb" id="fe-download-btn" title="Baixar"><i class="bi bi-download"></i></button>' +
                        '<button class="btn btn-xs btn-outline-danger fe-tb" id="fe-delete-btn" title="Excluir"><i class="bi bi-trash"></i></button>' +
                        '<button class="btn btn-xs btn-outline-light fe-tb" id="fe-refresh-btn" title="Atualizar"><i class="bi bi-arrow-clockwise"></i></button>' +
                    '</div>' +
...
function loadTree() {
    apiCall('list', {}, 'GET').then(function (data) {
        if (data.error) {
            alert('Erro ao carregar árvore: ' + data.result);
            return;
        }
...
function openFile(filePath) {
    if (hasUnsavedChanges() && !confirm('Você tem alterações não salvas. Descartar?')) return Promise.resolve(false);
    showEditorLoading('Carregando arquivo...');
    ...
    if (binaryExts.indexOf(ext) !== -1) {
        alert('Arquivo binário — não é possível editar: ' + filePath);
        return Promise.resolve(false);
    }

    return apiCall('read', { file: filePath }, 'GET').then(function (data) {
        if (data.error) {
            alert('Erro ao ler arquivo: ' + data.result);
            return false;
        }
        currentFile = filePath;
        document.getElementById('fe-current-file').textContent = filePath;
...
function setEditorContent(content, ext) {
    ...
    if (!bundle) {
        container.textContent = 'CodeMirror não carregado';
        return;
    }
...
function saveFile() {
    ...
    apiCall('save', { file: currentFile, content: content }).then(function (data) {
        if (data.error) {
            alert('Erro ao salvar: ' + data.result);
        } else {
            isDirty = false;
            initialContent = normalizeEditorContent(content);
...
function setupToolbar() {
    ...
    document.getElementById('fe-close-btn').addEventListener('click', function () {
        if (hasUnsavedChanges() && !confirm('Você tem alterações não salvas. Fechar mesmo assim?')) return;
    ...
    document.getElementById('fe-new-file').addEventListener('click', function () {
        ...
        var name = prompt('Nome do novo arquivo:', '');
    ...
    document.getElementById('fe-rename-btn').addEventListener('click', function () {
        var selPath = getSelectedPath();
        if (!selPath) { alert('Selecione um arquivo ou pasta primeiro'); return; }
    ...
    document.getElementById('fe-download-btn').addEventListener('click', function () {
        var selPath = getSelectedPath();
        var selType = getSelectedType();
        if (!selPath || selType === 'dir') { alert('Selecione um arquivo primeiro'); return; }
    ...
    document.getElementById('fe-delete-btn').addEventListener('click', function () {
        var selPath = getSelectedPath();
        if (!selPath) { alert('Selecione um arquivo ou pasta primeiro'); return; }
        if (!confirm('Excluir "' + selPath + '"?')) return;
...
export function openFileEditor(folderName, type) {
    ...
    showEditorLoading('Carregando arquivos...');
    loadTree();
}
