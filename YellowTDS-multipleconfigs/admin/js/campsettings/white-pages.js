...
function buildWhiteFolderRow(folderName, mode) {
    ...
    return '<div class="form-group-inner white-folder-item"><div class="row">' +
        ...
        '<a href="javascript:void(0)" class="btn btn-outline-secondary load-mode-btn" data-mode="' + mode + '" data-modes="base,rewrite,direct" title="Modo de carregamento"><i class="bi ' + icon + '"></i></a>' +
        '<a href="javascript:void(0)" class="btn btn-warning white-edit-folder" title="Editar arquivos"><i class="bi bi-pencil-square"></i></a>' +
        '<a href="javascript:void(0)" class="btn btn-danger remove-white-folder-item" title="Excluir"><i class="bi bi-trash"></i></a>' +
        '</div></div></div></div>';
}
...
document.querySelector('.white-add-existing')?.addEventListener('click', function () {
    ...
    fetch('listfolders.php?type=white').then(function (r) { return r.json(); }).then(function (data) {
        ...
        if (!data.folders.length) { alert('Nenhuma pasta de página segura encontrada. Envie um ZIP primeiro.'); return; }
...
document.querySelector('.white-upload-zip')?.addEventListener('click', function () {
    ...
    fileInput.addEventListener('change', function () {
        ...
        var folderName = prompt('Digite o nome da pasta para a nova página segura:');
