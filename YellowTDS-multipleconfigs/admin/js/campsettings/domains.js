...
function buildDomainRow(domain, statusHtml) {
    return '<div class="form-group-inner domain-item"><div class="row">' +
        ...
        '<div class="col-lg-3"><input type="text" class="form-control domain-name" value="' + domain + '" placeholder="dominio.com" readonly /></div>' +
...
function domainStatusIcon(data) {
    if (data.wildcard) {
        return '<i class="bi bi-asterisk domain-status" style="color:#f59e0b" title="Domínio curinga — verificação DNS ignorada"></i>';
    }
    if (data.cloudflare) {
        return '<span class="domain-cf-badge domain-status" title="CloudFlare detectado (IP: ' + (data.ip || '?') + ')">CF</span>';
    }
    if (data.resolves) {
        return '<i class="bi bi-check-circle-fill domain-status" style="color:#22c55e" title="Resolve para este servidor (' + (data.ip || '') + ')"></i>';
    }
    var errMsg = data.error || 'O domínio não resolve para este servidor';
...
function checkDomainStatus(row) {
    ...
    col.innerHTML = '<i class="bi bi-hourglass-split domain-status" style="color:#94a3b8" title="Verificando..."></i>';
    checkDomain(domain).then(function (data) {
        col.innerHTML = domainStatusIcon(data);
    }).catch(function () {
        col.innerHTML = '<i class="bi bi-question-circle domain-status" style="color:#94a3b8" title="Falha na verificação"></i>';
    });
}
...
document.getElementById('add-domain-item')?.addEventListener('click', function () {
    var domain = prompt('Digite o domínio (sem http(s)://):');
    ...
    for (var i = 0; i < existing.length; i++) {
        if (existing[i].value.trim().toLowerCase() === domain.toLowerCase()) {
            alert('O domínio "' + domain + '" já foi adicionado.');
            return;
        }
    }

    var loadingHtml = '<i class="bi bi-hourglass-split domain-status" style="color:#94a3b8" title="Verificando..."></i>';
