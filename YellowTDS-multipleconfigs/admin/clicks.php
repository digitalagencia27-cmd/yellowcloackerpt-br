        ...
        <div class="buttons-block" style="display: flex; justify-content: space-between; align-items: center;">
            ...
            <?php if ($view !== 'trafficback'):?>
                <span style="display: inline-block;">
                    <label for="viewSelector" style="color: white; font-weight: bold;">Visualização:</label>
                    <select id="viewSelector" class="form-select" style="width: 140px; display: inline-block; margin-left: 10px;">
                        <option value="allowed"<?= $view == 'allowed' ? ' selected' : '' ?>>Permitidos</option>
                        <option value="blocked"<?= $view == 'blocked' ? ' selected' : '' ?>>Bloqueados</option>
            ...
            <?php if ($showIdSearch): ?>
                <form id="idSearchForm" method="get" action="clicks.php" style="display:inline-flex; align-items:flex-end; gap:8px; margin-left: 8px;">
                    ...
                    <span style="display:inline-flex; align-items:center; gap:6px;">
                        <label for="idSearchInput" style="color: white; font-weight: bold; margin-bottom:0;">Buscar:</label>
                        <input
                            id="idSearchInput"
                            name="search"
                            type="text"
                            ...
                            placeholder="ID do Usuário / ID do Clique"
                            style="width: 240px;">
                    </span>
                </form>
            <?php endif; ?>
            ...
            <div>
                <button id="resetFilters" title="Resetar todos os filtros" class="btn btn-outline-danger" style="margin-left: 8px;<?= $hasActiveFilters ? '' : ' display:none;' ?>"><i
                        class="bi bi-funnel"></i> Resetar Filtros</button>
                <button id="columnsSelect" title="Selecionar e ordenar colunas" class="btn btn-info" style="margin-left: 8px;"><i
                        class="bi bi-layout-three-columns"></i></button>
                <button id="downloadCsv" title="Baixar tabela como XLSX" class="btn btn-success" style="margin-left: 8px;"><i
