...
function traficback(array $clickParams): CloakerAction
{
    ...
    return empty($tbUrl) ?
        new CloakerAction('traficback', 'die', 'NENHUMA CAMPANHA PARA ESTE DOMÍNIO E TRAFFICBACK NÃO DEFINIDO!') :
...
function black(Campaign $c, int $flowIndex, array $clickparams): CloakerAction
{
    ...
    if (empty($steps)) {
            return new CloakerAction('black', 'die', "Nenhuma etapa definida no fluxo: " . $flow->name);
    }
    ...
    if (!is_valid_planned_path($plannedPath, $steps)) {
            return new CloakerAction('black', 'die', "Caminho planejado inválido para o fluxo: " . $flow->name);
    }
    ...
    if (!$db->add_black_click($userid, $clickid, $clickparams, $plannedPath, $flow->name, $c->campaignId)) {
        return new CloakerAction('black', 'die', 'Falha ao registrar clique');
    }
    if (!$db->add_click_step($clickid, 0, $plannedPath[0])) {
        return new CloakerAction('black', 'die', 'Falha ao registrar entrada de etapa');
    }
