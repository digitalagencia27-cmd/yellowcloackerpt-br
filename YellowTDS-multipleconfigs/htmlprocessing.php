...
//se o tipo do campo de telefone for text, mudar para tel para entrada mais conveniente no mobile
//adicionar autocomplete aos campos de nome e telefone
//adicionar required se não estiver presente
function fix_phone_and_name($html)
{
    //corrigir type=text para type=tel
...
//carregar página branca da PASTA
function load_white_content($url, string $mode = 'base'): string
{
    ...
    //adicionando no-referer,noindex,nofollow
...
//sanitizar HTML da página branca: remover rastreadores, og:url, canonical, noscript; adicionar noindex/nofollow
function sanitize_white_html(string $html): string
{
    //remover tudo desnecessário
    $html = preg_replace('/(<meta property=\"og:url\" [^>]+>)/', "", $html);
    $html = preg_replace('/(<link rel=\"canonical\" [^>]+>)/', "", $html);
    //eliminando scripts de rastreamento
    ...
    //removendo todas as tags noscript
    $pattern = '#<noscript>.*?</noscript>#is';
    $html = preg_replace($pattern, '', $html);
    //adicionando algumas tags adicionais ao head
    $html = str_replace('<head>', '<head><meta name="referrer" content="no-referrer"><meta name="robots" content="noindex, nofollow">', $html);

    return $html;
}

//carregando página branca com CURL
...
//reescrever urls relativas (não começando com http ou //)
