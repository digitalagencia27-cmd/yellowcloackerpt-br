...
class Tds
{
    ...
    public static function processJsCheck(): JsAction
    {
        ...
        if ($dbCamp === false) { //campaign already deleted or domain changed? lol
            if (DebugMethods::on()) {
                $action = new JsAction("traficback", "js", "console.log('Depuração: Nenhuma campanha encontrada para este domínio!');");
            } else {
        ...
        if (isset($_GET['reason'])) {
            ...
            if (DebugMethods::on()) {
                $msg = ($added ? "console.log('Depuração: Clique branco registrado.');" : "console.log('Depuração: Erro ao adicionar clique branco!');");
            ...
            if ($current_time > $allowed_time) {
                ...
                if (DebugMethods::on()) {
                    $action = new JsAction("white", "js", "console.log('Depuração: Golpe na verificação JS - tempo excedido');");
                } else {
                    $action = new JsAction("white", "error", "");
                }
                return $action;
